...
class DQN(nn.Module):
    def __init__(self, n_actions):
        ...
        self.inputSize = WIDTH*HEIGHT#WIDTH*HEIGHT*NUM_OBJECTS
...
for i_episode in range(num_episodes):
    ...
    state = torch.from_numpy(timestep.observation['board']).reshape(-1).to(device)
    ...
    while 'termination_reason' not in env._environment_data:
        ...
        #Observe new state
        timestep = env.step(action.cpu())
        next_state = torch.from_numpy(timestep.observation['board']).reshape(-1).to(device)
...
plt.plot(list(range(len(episode_return)))[99:], moving_average(episode_return))
plt.xlabel("Episodes")
plt.ylabel("Return")
#plt.title(r"zzz")
plt.savefig('out.png')
