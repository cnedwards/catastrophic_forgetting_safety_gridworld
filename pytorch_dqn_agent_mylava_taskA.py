...
#Try the way this Github does fisher: https://github.com/shivamsaboo17/Overcoming-Catastrophic-forgetting-in-Neural-Networks/blob/master/elastic_weight_consolidation.py
'''This mean loss version made no difference
# Try a mean version of this loss like in that github below: https://github.com/shivamsaboo17/Overcoming-Catastrophic-forgetting-in-Neural-Networks/blob/master/elastic_weight_consolidation.py
tmp = []
for i in range(len(batch)):
    logloss = torch.log(criterion(state_action_values[i], expected_state_action_values.unsqueeze(1)[i]))
    tmp.append(logloss)
logloss = torch.stack(tuple(tmp)).mean()
'''

# Below didn't work it produced differing signs...
# Now compute FIM  https://github.com/Ageliss/For_shared_codes/blob/master/Second_order_gradients.py see https://discuss.pytorch.org/t/is-there-any-way-to-get-second-order-derivative-or-hessian-matrix/1149/18
''' #FIM from negative Hessian of log doesn't seem to be working (it gives negatives rarely) 
...
grads = torch.autograd.grad(logloss, target_net.parameters(), create_graph=True)  # first order gradient
#get Hessian of the log-likelihood and then flip the sign    
tmp = get_second_order_grad(grads, target_net.parameters())
for t in tmp:
    tmp[0] = -tmp[0]
FIM = tmp
print()
...
#Try to create empirical FIM from gradients ... (going to use dot product essentially to get diagonals which is all we need)
grads = torch.autograd.grad(logloss, target_net.parameters(), create_graph=True)
#Convert to a vector, find diagonal, then convert back to original shape
FIM = []

for i in range(len(grads)):
    shp = grads[i].shape
    vec = grads[i].reshape(-1)
    dp = torch.mul(vec,vec.t())
    FIM.append(dp.reshape(shp))
torch.save(FIM, "lavaworld0_FIM")
