...
while steps_done < MAX_TIMESTEPS: 
    ...
    while not env.check_end(): # While not a terminal state and not max iterations
        ...
        memory.push(state, action, next_state, reward, env.check_terminal_state_one_hot(next_state.numpy().reshape(HEIGHT,WIDTH,NUM_OBJECTS)))
...
#Save parameters:
torch.save(target_net.state_dict(), "lavaworld0+1_parameters")
